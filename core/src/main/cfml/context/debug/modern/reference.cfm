...
";
				...
				if(LuceeAIHas(endpoint)) {
					endpointName=LuceeAIGetNameForDefault("documentation");
					if(!structKeyExists(session, "documentationAISession") || (session.documentationAISession.name?:"") != endpointName) {
						...
						session.documentationAISession={"ais":ais,"label":label,"name":endpointName};
					}
					else {
