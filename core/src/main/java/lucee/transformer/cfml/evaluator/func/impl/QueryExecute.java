...
public final class QueryExecute implements FunctionEvaluator {
	...
	private void addArgument(BIF bif, String str) {
		...
		if (args[0] instanceof NamedArgument) {
			bif.addArgument(new NamedArgumentImpl(bif.getFactory().createLitString("name"), bif.getFactory().createLitString(str), "string", false));
		}
		// regular arguments
		else {
			// add params
			if (args.length == 1) {
				bif.addArgument(new ArgumentImpl(bif.getFactory().createNull(), "any"));
			}
			// add options
			if (args.length <= 2) {
				bif.addArgument(new ArgumentImpl(bif.getFactory().createStruct(), "struct"));
			}
			// add the name
			bif.addArgument(new ArgumentImpl(bif.getFactory().createLitString(str), "string"));
		}
	}
