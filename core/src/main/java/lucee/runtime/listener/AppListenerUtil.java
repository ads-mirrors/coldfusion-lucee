...
public final class AppListenerUtil {
	...
	public static DataSource toDataSource(Config config, String name, Struct data, Log log) throws PageException {
		...
		try {
			...
			return new DataSourceImpl(config, name, dbt.classDefinition, Caster.toString(data.get(KeyConstants._host)), dbt.connectionString, Caster.toString(data.get(KeyConstants._BUNDLENAME)), Caster.toString(data.get(KeyConstants._BUNDLEVERSION)),
