...
public final class SetDay extends BIF {
	...
	public Object invoke(PageContext pc, Object[] args) throws PageException {
		if (args.length < 2 || args.length > 3) throw new FunctionException(pc, "SetDay", 2, 3, args.length);
	...
	private static DateTime _call(DateTime date, int value, TimeZone tz) {
		DateTimeUtil.getInstance().setDay(tz, date, value);
		return date;
	}
