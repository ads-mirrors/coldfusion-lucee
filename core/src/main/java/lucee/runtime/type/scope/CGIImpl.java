...
public final class CGIImpl extends StructSupport implements CGI, ScriptProtected, Externalizable {
	...
	private static Struct staticKeys = new StructImpl(StructImpl.DEFAULT_TYPE, 64);
	static {
	...
	public void initialize(PageContext pc) {
		...
		// if(internal==null) {
		internal = new StructImpl(StructImpl.DEFAULT_TYPE, 64);
		...
		if (req != null) {
			...
			try {
				if (first == 'a') {
					...
					if (key.equals(KeyConstants._request_uri)) return store(key, toString(req.getAttribute("jakarta.servlet.include.request_uri")));/* JAVJAK */
