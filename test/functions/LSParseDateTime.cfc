	...
	function run( testResults , testBox ) {
		describe( title="Test suite for LSparseDateTime()", body=function() {
			...
			it( title="checking LSparseDateTime() French (swiss)", body = function( currentSpec ) {
				...
				assertEquals("-{ts '2008-04-06 01:02:00'}", "-#lsParseDateTime("6 avr. 2008 01:02")#");
				// the following all fail on windows / java 11 
			...
			it( title="checking LSparseDateTime() English (US)", body = function( currentSpec ) {
				...
				assertEquals("-{ts '2008-04-06 01:02:00'}", "-#lsParseDateTime(date:"4/6/08 1:02 AM",timezone:"CET")#");
			...
			it( title="checking LSparseDateTime() misc tests", body = function( currentSpec ) {
