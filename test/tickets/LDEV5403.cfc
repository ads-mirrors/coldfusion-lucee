component extends="org.lucee.cfml.test.LuceeTestCase" {
	...
	function run( testResults, testBox ) {
		...
		describe("LDEV-5403 remove support for loginStorage=cookie and sessionStorage=cookie", function() {
			...
			it( title="check loginStorage=cookie works in 6",skip=true, body=function( currentSpec ) {
			...
			it( title="check sessionStorage=cookie works in 6",skip=true,  body=function( currentSpec ) {
				var result = _InternalRequest(
