component extends="org.lucee.cfml.test.LuceeTestCase" labels="orm"{
	function run( testResults , testBox ) {
		describe( title="Test suite for LDEV-1659", body=function() {
			it( title='Initialize ORM Secondary cache', skip=noOrm(), body=function( currentSpec ) {
			...
			it( title='Checking ORM cache connection', skip=noOrm(), body=function( currentSpec ) {
